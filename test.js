...
function App() {
  ...
  const Dropdown = ({ label, options, value, onChange }) => {
    ...
    return (
      <div>
        ...
        {showOptions && (
          <ul style={{ position: 'absolute', zIndex: 1, listStyle: 'none', padding: 0, margin: 0, backgroundColor: 'white', border: '1px solid #ccc' }}>
            {options.map((option) => (
              <li key={option} onClick={() => handleOptionClick(option)} style={{ padding: '5px 10px', cursor: 'pointer' }}>
                {option}
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="App">
      ...
      <div>Compute Engine Subtotal: ${subtotal.toFixed(2)}</div>
    </div>
  );
}
